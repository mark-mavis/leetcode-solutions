using HelperObjects;

class MediumController{
    public static void Run(){
        ListNode n3 = new (3, null);
        ListNode n2 = new (4, n3);
        ListNode head1 = new (2, n2);

        ListNode n6 = new ListNode(4, null);
        ListNode n5 = new ListNode(6, n6);
        ListNode n4 = new ListNode(1, n5);
        ListNode head2 = new ListNode(5, n5);

        //Medium.AddTwoNumbers.Solution(head1, head2);

        //Medium.LongestSubstringWithoutRepeatingCharacters.BruteForce("abcabcbb");
        Medium.LongestSubstringWithoutRepeatingCharacters.SlidingWindows("abcabcbb");



    }
}